...
## 1. 개요 및 최종 목표
* **목표 1**: 실물화상기 특성에 맞춰 앱 구동 시 화면을 기본적으로 좌우반전된 상태(`isFlipped = true`)로 시작.
...
### 태스크 1: 기본 좌우반전 설정
* **설명**: 앱 실행 및 변형 초기화 시 기본 반전 상태 활성화.
* **수정 파일**:
  * [src/hooks/useViewerTransform.ts](file:///d:/codes/WebcamViewer/src/hooks/useViewerTransform.ts)
* **내용**:
  * `isFlipped` 상태의 초기값(`useState`)을 `true`로 설정.
  * `resetTransform` 콜백 내에서 `setIsFlipped(true)`로 리셋 설정.
* **인수 조건**: 앱 시작 시 또는 초기화 단축키(0) 입력 시 화면이 반전 상태로 기동함.
...
* **내용**:
  ...
  * `triggerAutoFocus` 실행 시 웹캠 미디어 트랙의 capabilities를 조회하여 `single-shot` 또는 `continuous` 모드 전환 조정을 수행해 카메라의 오토포커싱 웜업(재초정)을 유도함.
* **인수 조건**: 자동 초점 기능을 지원하는 장치 감지 시 `isAutoFocusSupported`가 true가 됨.
