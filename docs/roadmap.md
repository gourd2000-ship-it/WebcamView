...
### Phase 5: Packaging & Release (패키징 및 배포) ✅ 완료
- **상태**: 100% 완료
- **목표**: Windows 실행 파일(`.exe`) 패키징 검증 및 가이드 작성.
- **주요 작업**:
  - `electron-builder` 구성 (아이콘, 파일 쓰기 권한 정의 등) 완료.
  - 로컬 Windows 환경 배포판(`.exe` 및 nsis 설치 파일) 빌드 테스트 성공 완료.
  - `README.md`에 설치, 실행, 빌드 방법, 단축키 표 및 테스트 가이드 상세화 완료.
- **결과물**: "인터넷 없이도 설치하고 바로 실행할 수 있는 무설치형 포터블 `.exe` 파일".

### Phase 6: Overlay Annotation (화면 판서 및 주석) ✅ 완료
- **상태**: 100% 완료
- **목표**: 비디오 위에 그리는 펜, 형광펜, 지우개, 간단한 도형(원, 사각형, 화살표) 툴바 추가 및 판서 포함 캡처 저장.
- **주요 작업**:
  - `AnnotationCanvas` 레이어 구축: 비디오 뷰와 1:1 종속되는 Canvas 오버레이 마운트 완료.
  - 플로팅 주석 툴바(`AnnotationToolbar`) 마운트: 펜/형광펜/지우개 및 선/원/사각형/화살표 그리기 기능 완료.
  - 마우스 포인터 선택 시 드래그 이동(Pan)이 활성화되며, 그리기 도구 선택 시 판서 모드가 자동으로 활성화되는 직관적 스위칭 구현 완료.
  - 저장 시 판서 레이어를 영상 캔버스와 1:1 병합하여 고해상도 PNG 파일로 함께 저장 완료.
- **결과물**: "실시간 카메라 영상 위에 마우스/터치 펜으로 즉시 판서하고, 판서와 영상이 하나로 합쳐진 스냅샷 파일 저장 기능".

### Phase 7: Image Adjustments (영상 보정 필터) ✅ 완료
- **상태**: 100% 완료
- **목표**: 대비(Contrast), 밝기(Brightness), 흑백 반전(전자칠판용 서류 모드), 흑백 모드 필터 및 슬라이더 설정 카드 구현.
- **주요 작업**:
  - CSS 필터 속성 연동: 비디오 및 이미지 요소를 대상으로 GPU 가속 밝기/대비/색상반전/그레이스케일 렌더링 완료.
  - 슬라이딩 설정 패널 마운트: 툴바와 연동되어 밝기(50%~200%), 대비(50%~200%), 색상 반전 토글, 흑백 모드 토글 및 필터 일괄 초기화 기능 완료.
  - 필터 캡처 동기화: 캔버스 2D 컨텍스트의 `ctx.filter` 속성을 이용하여 실시간 보정된 화질 상태 그대로 고해상도 PNG 파일로 함께 저장 완료.
  - 필기 잉크 분리: 필터 연산을 비디오에만 한정시켜, 색상 반전 시에도 판서의 필기 색상(적/청 등)은 원색 그대로 정상 렌더링되도록 격리 설계 완료.
- **결과물**: "실시간 밝기/대비 제어 및 칠판 전용 흑백 반전 모드를 제공하는 고해상도 화질 보정 카메라".
...
1. **Phase 8: 사용자 환경 설정 보존 (Settings Persistence)**
