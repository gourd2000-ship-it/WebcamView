...
- **주요 작업**:
  - `package.json`, `vite.config.ts`, `tsconfig.json` 설정.
  - Electron Main 프로세스 및 Preload 브릿지 선언.
  - 다크 테마 기반의 반응형 레이아웃 마크업 (상단 상태 바, 메인 뷰어 영역, 하단 툴바).
  - UI 공통 버튼 컴포넌트(`IconButton`) 및 에러/로딩/빈 상태 UI 구축.
- **결과물**: "개발 서버 실행 시 기본 UI 프레임이 나타나는 데스크톱 창 구동".

### Phase 2: Core Camera & View Transform (카메라 연동 및 조작)
- **목표**: 웹캠 장치 검색, 라이브 비디오 스트리밍, 그리고 CSS 기반의 실시간 화면 조작 구현.
- **주요 작업**:
  - `useCamera` 훅 구현: `navigator.mediaDevices`를 활용하여 디바이스 나열 및 카메라 전환 지원.
  - `CameraSelector` 드롭다운 컴포넌트 연동.
  - **카메라 생명주기 관리**: 카메라 중지 및 장치 전환 시 기존 `MediaStreamTrack`을 명시적으로 `.stop()` 처리하는 메모리 누수 방지 유틸 적용.
  - `useViewerTransform` 훅 구현: 좌우반전(`scaleX(-1)`), 90도 회전(`rotate`), 확대/축소(`scale`) CSS transform 값 매핑.
- **결과물**: "연결된 카메라 목록을 선택해 화면에 띄우고, 마우스 클릭으로 회전/반전/확대할 수 있는 라이브 뷰어".

### Phase 3: Freeze & Local Capture (화면 정지 및 고해상도 저장)
- **목표**: 비디오 일시 정지(Freeze) 기능 및 변환 매트릭스가 반영된 고해상도 로컬 이미지 캡처/저장.
- **주요 작업**:
  - **화면 정지**: 현재 `<video>` 프레임을 canvas에 순간 캡처하여 데이터 URL화하고, `<img src={...} />` 요소로 전환하여 정지 구현 (이때도 CSS 변환은 동일 유지).
  - **Canvas 이미지 렌더링**: CSS 변환 상태(회전, 반전, 확대)와 1:1 대응되는 좌표 매핑을 적용하여 오프스크린 Canvas에 현재 프레임을 그리는 헬퍼 구현.
  - **로컬 디스크 저장**: Preload 브릿지의 `window.electronAPI.saveCapture`를 통해 바이너리(ArrayBuffer)를 Main 프로세스로 전달, Main에서 Node.js `fs`를 이용해 `webcam-capture-YYYY-MM-DD-HHMMSS.png` 형태로 로컬 무인 저장.
- **결과물**: "원터치 화면 정지/해제 및 회전/반전/배율이 정확히 보정된 스크린샷 로컬 저장 기능".

### Phase 4: Keyboard Shortcuts & Hardening (단축키 및 안정화)
- **목표**: 전체화면 포함 환경에서의 전역 단축키 수신 및 예외 처리 강화.
- **주요 작업**:
  - `useFullscreen` 훅 구현: HTML5 Fullscreen API 연동 및 `Esc` 예외 대응.
  - `useKeyboardShortcuts` 훅 구현: `document` 레벨에서 캡처 단계(`useCapture: true`) 리스너를 바인딩하여 포커스가 없거나 전체화면이어도 단축키(`Space`, `F`, `M`, `R`, `+`, `-`, `0`, `C`, `Esc`)가 동작하도록 처리.
  - 장치 점유 상태 예외, 권한 거부 예외, 저장 실패 예외 대응 UI 팝업 연동.
- **결과물**: "실수 없는 키보드 핫키 처리 및 비정상 하드웨어 상태 예외에 강한 완성도 높은 클라이언트".

### Phase 5: Packaging & Release (패키징 및 배포)
